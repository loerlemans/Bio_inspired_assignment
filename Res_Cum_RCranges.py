# run_uav_coverage_simulation.py
...
def main():
    ...
    for num_uavs in num_uavs_list:
        print(f"Running simulation for {num_uavs} UAVs")
        coverage_results[num_uavs] = run_simulation(num_uavs)

    # Plotting the results
    plt.figure(figsize=(10, 6))
    for num_uavs in num_uavs_list:
        plt.plot(coverage_results[num_uavs], label=f'{num_uavs} UAVs')

    plt.xlabel('Time (s)')
    plt.ylabel('Cumulative Area Coverage (%)')
    plt.title('Cumulative Area Coverage vs Time for Different Numbers of UAVs')
    plt.legend()
    plt.grid(True)
    plt.show()
